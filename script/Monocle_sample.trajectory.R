...
CairoPNG (file = paste(c(basic.path, "plot_cell_trajectory_orig.png"), collapse=""), width = 800, height=600)
plot_cell_trajectory(sample.subset.OC, color_by = "res.0.6")
dev.off()

CairoPNG (file = paste(c(basic.path, "plot_cell_trajectory_pseudotime.png"), collapse=""), width = 800, height=600)
plot_cell_trajectory(sample.subset.OC, color_by = "Pseudotime")
dev.off()

###Save RDS file
saveRDS (clustering_DEG_genes, file = paste(c(output.path,"/Rdata/",sample.id,".clustering_DEG_genes.Rda"), collapse=""))
